...
#    镜像已推送 Docker Hub（jerryvon/opc-backend/opc-frontend:v2.5+），
#    本地无对应 tag 时 compose 可直接拉取；强制构建用 docker compose build
if ! docker image inspect jerryvon/opc-backend:v2.5 >/dev/null 2>&1; then
  echo "[start-all] 未找到本地 v2.5 镜像，尝试从 Docker Hub 拉取..."
  docker pull jerryvon/opc-backend:v2.5 || { echo "[start-all] 拉取失败（网络受限？），改用源码构建（首次需几分钟）..."; docker compose build; }
fi
