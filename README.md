...
- 👤 **成员体系**：两步注册（密码 + 邮箱验证码，成员编号**后端自动生成**）、登录（证号/手机号/邮箱 + BCrypt 密码，三层频控）、会话 token（Redis **14 天滑动续期**）校验归属、登录审计日志
...
> 2026-08-24 起按产品规划精简：移除公告类「逾期检查/逾期催办公告/公告回收」三个任务（公告系统已下线），仅保留候补释放。
...
- 管理后台：`http://localhost/index.html`（默认账号 admin / admin123（当前实际口令，首次登录请修改））
...
- 管理后台：`http://localhost:8081/index.html`（默认账号 admin / admin123（当前实际口令，首次登录请修改））
...
# 只需额外执行增量升级脚本（幂等）：
mysql --default-character-set=utf8mb4 -uroot -p ry-vue < sql/upgrade_20260821_01_official.sql   # 官网化业务重映射（服务/成员/报名/候补/入驻申请）
...
- 后台地址：`http://localhost:8081/`（默认账号 admin / admin123（当前实际口令））
...
前台成员登录后由后端签发 **会话 token**（存 Redis，默认 14 天滑动续期，`sys_config` 的 `reader.session.minutes` 可配），我的报名/候补/入驻申请查询与报名/延期/取消等操作均需携带 `X-Session-Token` 请求头并由后端校验归属（token 绑定 cardNo），不再信任前端提交的成员编号，降低冒用他人编号的风险。密码重置/管理员代设密后全端会话自动吊销。未登录时相关入口会提示先登录。
...
- [《交接须知》](docs/交接须知.md)：**接手必读红线清单**（命名映射/踩坑/自检）
...
| `2026-08-24` | **认证与邮件整体改造**（M1-M5，`8e87455f` 等）：前台登录改证号/手机号/邮箱三标识 + BCrypt 密码；两步注册（邮箱验证码）+ 忘记密码找回；会话 14 天滑动续期 + 多端管理；三层频控（IP 限速 → 账号锁定 → 递增退避）+ 登录审计日志；邮件后台可视化配置（SMTP/模板，system:mail:* 权限）；个人主页账号安全区（改密/改邮箱/设备管理） |
| `2026-08-25` | **前台脚本统一与导航重构**（`1ae924c6`~`faffe104`）：12 页公共 JS 收敛为 `js/site.js` 单一来源；navbar 全站一致 + 按 URL 自动高亮 + 滚动联动；修复多页脚本语法事故（moreMenu 监听/悬空 token） |
| `2026-08-25` | **后台菜单重组与回收站返工**（`f61c8ede`~`942ecf03`）：业务导向四分组（内容运营/成员与报名/运营辅助/系统设置）；回收站入口删除→恢复→更名（服务回收站/成员回收站）；全库术语清零（图书→服务/读者→成员/借书→报名） |
| `2026-08-25` | **第五轮全面审查 + 修复**（`09fe0358`~`7f035c92`）：4 路并行审查后修复 H2 升级链统一、H3 editor 角色（补建 14 个按钮权限菜单 + 新目录重授，`upgrade_20260825_05_editor_fix.sql`）、H4 后台路由、H5 CMS 发布权限后端强制、H7 前台死点，及 M1-M4/M6-M9/M11/M12/M14（欠费删除守卫/栏目孤儿文章/忘记密码防枚举/上传链路/校验一致性/回收站 loading/ECharts 泄漏等）；**CI 单测修复 114/114 全绿**（认证改造后测试同步）；home 死弹窗清理（-130 行）；H1 经核实为误报（JWT 启动自检有效） |
| `2026-08-25` | **安全收口与文档整理**：admin/editor 口令随机化（种子 SQL + 本地库同步，BCrypt 验证）；文档合并去重（改造方案→开发文档附录、修改建议→TODO）+ 新增《交接须知》 |
