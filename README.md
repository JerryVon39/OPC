...
8 大栏目：**首页**（`index.html`）/ **走进社区**（`about.html`）/ **产业生态**（`industry.html`）/ **政策赋能**（`policy.html`）/ **入驻招商**（`join.html`）/ **人才培养**（`talent.html`）/ **新闻动态**（`news.html`）/ **联系我们**（首页与服务中心锚点），全部为真实社区信息（来源：《官网内容库》，与公开报道交叉验证）

- 🧩 **服务中心**（`shop.html`）：社区 AI 服务展示，支持按服务名称/主办方/服务编号**搜索**、搜索**联想**（防抖 200ms / ↑↓ 选择 / 回车选中）、分类 Tab 切换与排序
- 📝 **入驻申请**：前台提交入驻意向（项目/组织名称 + 联系人 + 申请说明 + 邮箱）→ 后台处理（待处理/已处理/已拒绝，同名称防重复）
- ⚡ **乐观 UI**：报名 / 延期 / 候补点击瞬间按钮即变，出错自动回滚；锁粒度按服务拆分，并行操作互不阻塞
- 🖼️ **服务展示**：竖版卡片（后台可上传封面）、在架/下架/名额紧张标签（阈值后台可配）、热门推荐 Top5
- 👤 **成员体系**：自助注册（成员编号**后端自动生成**，防伪造/占坑）、登录（**电子邮箱必填**，用于报名/候补等自动邮件通知，个人中心可改）、挂失/停用证号自动拒绝
- 📋 **我的报名**：查看剩余天数、自助延期；**满员候补**：名额为 0 时进入候补队列，有名额自动邮件通知
- 📢 **公告横幅**：自动轮播 + 长公告跑马灯滚动 + 当日可关闭
- 🎠 **轮播图管理**：后台增删改前台轮播（上传图片/排序/启用停用）
- 📊 **数据条**：横幅下方实时统计（服务数/成员数/今日报名等）
- 📱 **手机自适应**：≤768px 自动切换移动版布局；前后台一键互跳
...
- 官网前台：`ruoyi-ui/public/shop.html`（服务中心）、`ruoyi-ui/public/index.html`（首页）等
