...
> ⏸️ **已停用业务**：原「服务业务」模块（服务信息/报名/候补/订单/挂失补办等）此前随产品方向调整**整体停用**——前台页面（services/service/contest.html）已删除，后台菜单（服务信息/服务回收站等）幂等隐藏（`upgrade_20260825_19_hide_book_menu.sql` + `upgrade_20260827_09`），代码/数据/接口保留可逆恢复。**报名功能重启已列为最高优先待办（TODO #1）**——恢复页面与菜单后即可复用现有报名/候补/订单链路。
