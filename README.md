...
- 🗂️ **文章管理 CMS**：文章增删改查/复制/批量置顶/批量下线/批量移动栏目/回收站还原、预约发布与**定时下线**（到点自动隐藏）、版本历史回滚、草稿自动保存、浏览量报表（Top20/栏目分布/近30天趋势，日粒度累计）
- 🧩 **区块管理（页面搭建核心）**：17 种模板（schema 驱动）+ 实时预览/高亮定位 + 未保存拦截 + 放大编辑 + 列表项排序 + 一键复制跨页复用 + 历史版本回滚；编辑区可拖拽调宽（偏好记忆）
- 🗂️ **栏目管理**：树形 3 级、文章数统计、搜索、同级上移下移、**前台归属页面可配置**（资讯/政策，替代名称前缀硬编码）、发文章直达编辑页
- 📢 **公告运营**：生效/失效时间窗（过期自动下架）、公告条点击看全文、后台过期标记、时间窗校验
- 🖼️ **官网轮播**：样式增强（背景/文字色/渐变/图片适配）、CSS 白名单校验、实时效果预览
- 📊 **内容统计报表**：浏览量 Top20、栏目分布占比、近 30 天趋势折线（2026-08-27 新增）
- ⚙️ **图片上传自动压缩**：>2MB 且宽>2000px 自动等比压缩（JPEG 0.8 质量，白底防透明变黑）
- 🔗 **OG 分享卡片 + sitemap.xml**：文章详情动态 og 标签（微信分享预览）、匿名 sitemap 接口供搜索引擎收录
- 🏗️ **自定义前台页面**（候选待排期）：cms_page 注册表 + 动态页渲染，运营可在区块管理新增前台分页
...
- 管理后台：`http://localhost/index.html`（默认账号 admin——⚠️ 口令因环境而异：Docker 全新部署初始口令 Ee606EcUQsgj（基础库原始值）；本地开发库已人工改为 admin123。首次登录请立即修改）
...
- 管理后台：`http://localhost:8081/index.html`（默认账号 admin——⚠️ 口令因环境而异：Docker 全新部署初始口令 Ee606EcUQsgj（基础库原始值）；本地开发库已人工改为 admin123。首次登录请立即修改）
...
- 后台地址：`http://localhost:8081/`（默认账号 admin——⚠️ 口令因环境而异：Docker 全新部署初始口令 Ee606EcUQsgj（基础库原始值）；本地开发库已人工改为 admin123。首次登录请立即修改）
...
### 2026-08-27 CMS 全面增强（第九轮审查 + 功能批次）
- 审查修复：页面搭建菜单下线、预约发布详情补时间窗、新增默认草稿、离开明示自动保存、回滚保留发布状态、首页新闻政策过滤（修 data 字段解析 bug）、公告过期标记等
- 六大功能：文章定时下线 / 一键复制（文章+区块）/ OG+sitemap / 图片自动压缩 / 浏览量报表 / 批量移动栏目
- 栏目管理六项增强 + 栏目前台归属页面可配置（front_page 字段，替代名称前缀硬编码）
- 区块管理体验：拖拽跟手修复、放大编辑、未保存拦截、Ctrl+S、列表项排序、预览当前效果
