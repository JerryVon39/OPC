...
| `2026-08-25` | **第五轮全面审查 + 修复**（`09fe0358`~`7f035c92`）：4 路并行审查后修复 H2 升级链统一、H3 editor 角色（补建 14 个按钮权限菜单 + 新目录重授，`upgrade_20260825_editor_fix.sql`）、H4 后台路由、H5 CMS 发布权限后端强制、H7 前台死点，及 M1-M4/M6-M9/M11/M12/M14（欠费删除守卫/栏目孤儿文章/忘记密码防枚举/上传链路/校验一致性/回收站 loading/ECharts 泄漏等）；**CI 单测修复 114/114 全绿**（认证改造后测试同步）；home 死弹窗清理（-130 行）；H1 经核实为误报（JWT 启动自检有效） |
