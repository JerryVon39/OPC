...
SET @has := (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='book' AND COLUMN_NAME='del_flag');
SET @ddl := IF(@has=0,
  'ALTER TABLE `book`
...
SET @has := (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='reader' AND COLUMN_NAME='del_flag');
SET @ddl := IF(@has=0,
  'ALTER TABLE `reader`
     ADD COLUMN `del_flag` char(1) NOT NULL DEFAULT ''0'' COMMENT ''删除标志（0存在 2删除）'' AFTER `status`,
     ADD COLUMN `deleted_by` varchar(64) DEFAULT '''' COMMENT ''删除人'' AFTER `del_flag`,
...
SET @has_book_recycle := (SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='book_recycle');
...
SET @has_reader_recycle := (SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='reader_recycle');
