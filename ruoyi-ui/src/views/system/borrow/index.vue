<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="borrowList">
      <el-table-column label="报名ID" align="center" prop="borrowId" width="80" />
      <el-table-column label="成员" align="center" min-width="140">
        <template slot-scope="scope">
          <span>{{ scope.row.readerName || 'ID:' + scope.row.readerId }}</span>
          <div style="font-size:12px;color:#999">{{ scope.row.cardNo }}</div>
        </template>
      </el-table-column>
      <el-table-column label="服务" align="center" prop="bookName" min-width="180">
        <template slot-scope="scope">
          <span>{{ scope.row.bookName || 'ID:' + scope.row.bookId }}</span>
        </template>
      </el-table-column>
      <el-table-column label="报名日期" align="center" prop="borrowDate" width="110" />
      <el-table-column label="截止日期" align="center" prop="dueDate" width="110" />
      <el-table-column label="完成日期" align="center" prop="returnDate" width="110" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="成员" prop="readerId">
          <el-select v-model="form.readerId" placeholder="请选择成员" filterable style="width:100%">
            <el-option v-for="r in readerOptions" :key="r.readerId" :label="r.readerName + '（' + (r.cardNo || '无编号') + '）'" :value="r.readerId" />
          </el-select>
        </el-form-item>
        <el-form-item label="服务" prop="bookId">
          <el-select v-model="form.bookId" placeholder="请选择服务（显示剩余名额）" filterable style="width:100%">
            <el-option v-for="b in bookOptions" :key="b.bookId" :label="b.bookName + '（剩余名额' + (b.stock || 0) + '）'" :value="b.bookId" :disabled="b.status !== '0' || b.stock <= 0" />
          </el-select>
        </el-form-item>
        <el-form-item label="报名日期" prop="borrowDate">
...
export default {
  ...
  data() {
    return {
      ...
      form: {},
      rules: {
        readerId: [{ required: true, message: "请选择成员", trigger: "change" }],
        bookId: [{ required: true, message: "请选择服务", trigger: "change" }]
      }
    }
  },
  created() {
  ...
  methods: {
    ...
    resetQuery() {
      // 重置时一并清掉跳转带来的成员/服务筛选，避免"看不见的筛选条件"
    ...
    handleRenew(row) {
      this.$modal.confirm('确认续期《' + (row.bookName || '') + '》吗？截止日期将顺延 30 天').then(() => {
        return renewBorrow(row.borrowId)
      }).then(() => {
        this.$modal.msgSuccess("续期成功，截止日期顺延 30 天")
        this.getList()
      }).catch(() => {})
    },
    handleReturn(row) {
      this.$modal.confirm('确认核销《' + (row.bookName || '') + '》吗？完成后名额自动+1').then(() => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        ...
        api(this.form).then(res => {
          this.$modal.msgSuccess(this.form.borrowId ? "修改成功" : "报名成功，名额已-1")
          this.open = false
          this.getList()
        })
      })
    ...
    handleDelete(row) {
      this.$modal.confirm('确认删除该报名记录吗？').then(() => {
